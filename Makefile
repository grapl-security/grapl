...
.PHONY: engagement-view
engagement-view:
	docker build \
		-f $(CURDIR)/src/js/engagement_view/Dockerfile.Makefile \
		--target grapl-engagement-view \
		-t grapl/grapl-engagement-view:$(TAG) \
		$(CURDIR)/src/js/engagement_view

.PHONY: build-engagement-view
build-engagement-view:
	docker build \
		-f $(CURDIR)/src/js/engagement_view/Dockerfile.Makefile \
		--target engagement-view-build \
		-t grapl/grapl-engagement-view-build:$(TAG) \
		$(CURDIR)/src/js/engagement_view
...
notebook:
	$(DOCKER_BUILD_PYTHON) \
		...
		-t grapl/grapl-notebook:$(TAG) \
		$(CURDIR)/src

.PHONY: build-notebook-test
build-notebook-test:
	$(DOCKER_BUILD_PYTHON) \
		--target grapl-notebook-test \
		-t grapl/grapl-notebook-test:$(TAG) \
		$(CURDIR)/src

.PHONY: test-notebook
test-unit-notebook: build-notebook-test
	docker run --rm -t \
		-v $(CURDIR)/etc:/grapl-etc:ro \
		-t grapl/grapl-notebook-test
...
.PHONY: build
build: build-rust build-python build-js-graphql-endpoint $(LOCAL_SERVICES)
...
.PHONY: aws
aws: ## Build sources (debug) and create zips
	$(MAKE) build-aws
	$(MAKE) zip
...
.PHONY: test-unit
test-unit: build-engagement-view build-rust-image build-test-python-all build-notebook-test  ## run unit tests
	UID=$(UID) GID=$(GID) docker-compose -f docker-compose.unit-tests.yml up
	ERR=0;\
...
.PHONY: up
up: aws-services $(LOCAL_SERVICES) 
...
test-integration: aws-services build-test-python-all $(LOCAL_SERVICES) cypress ## run integration tests
	docker-compose -f docker-compose.Makefile.yml up -d;\
	UID=$(UID) GID=$(GID) docker-compose -f docker-compose.integration-tests.yml up;\
	ERR=0;\
