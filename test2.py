...
class AuidQuery(DynamicNodeQuery):
    ...
    def with_auid(
        ...
        self.set_int_property_filter(
            "auid", _int_cmps("auid", eq=eq, gt=gt, lt=lt)
        )
        return self
...
class AuidAssumptionQuery(DynamicNodeQuery):
    ...
    def with_assumed_auid(
            self: 'NQ',
            assumed_auid_query: Optional['IAuidQuery'] = None
    ) -> 'NQ':
    ...
    def with_assuming_process(
            self: 'NQ',
            assuming_process_query: Optional['IProcessQuery'] = None
    ) -> 'NQ':
...
class AuidAssumptionView(DynamicNodeView):
    ...
    def get_assumed_timestamp(self) -> Optional[int]:
        if not self.assumed_timestamp:
            self.assumed_timestamp = cast(Optional[int], self.fetch_property("assumed_timestamp", int))
        return self.assumed_timestamp

    def get_assuming_process_id(self) -> Optional[int]:
        if not self.assuming_process_id:
            self.assuming_process_id = cast(Optional[int], self.fetch_property("assuming_process_id", int))
...
class AuidAssumptionExtendsAuidQuery(AuidQuery):
    def with_auid_assumptions(
            self: 'NQ',
            auid_assumptions_query: Optional['IAuidAssumptionQuery'] = None
    ) -> 'NQ':
        auid_assumptions = auid_assumptions_query or AuidAssumptionQuery()
        auid_assumptions.with_assumed_auid(
            cast('AuidQuery', self)
        )
...
class AuidAssumptionExtendsProcessQuery(ProcessQuery):
    def with_assumed_auid(
            self: 'NQ',
            assumed_auid_query: Optional['IAuidAssumptionQuery'] = None
    ) -> 'NQ':
        assumed_auid = assumed_auid_query or AuidAssumptionQuery()
        assumed_auid.with_assuming_process(
...
class AuidAssumptionExtendsAuidView(AuidView):
    def get_auid_assumptions(
        ...
        return cast('AuidAssumptionView', self.fetch_edge("~assumed_auid", AuidAssumptionView))
...
class AuidAssumptionExtendsProcessView(ProcessView):
    def get_assumed_auid(
        ...
        return cast('AuidAssumptionView', self.fetch_edge("~assuming_process", AuidAssumptionView))
