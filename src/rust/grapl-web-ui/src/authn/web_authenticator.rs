...
impl WebAuthenticator {
    ...
    pub(crate) async fn validate_session_token(
        ...
        let organization_id = uuid::Uuid::parse_str(user_row.get_organization_id()).map_err(|source| 
            AuthenticationError::ParseOrgId { input: user_row.get_organization_id().to_string(), source }
        )?;
