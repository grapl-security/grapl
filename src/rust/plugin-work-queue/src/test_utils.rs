...
pub async fn scan_for_generator_plugin_message_in_pwq(
    ...
    plugin_id: uuid::Uuid,
) -> eyre::Result<NextExecutionRequest> {
    ...
    let scan_thread = tokio::task::spawn(async move {
        ...
        tokio::time::timeout(timeout, scan_for_generator_job)
            .await
            .map_err(|_| {
                eyre::eyre!("did not find a matching Analyzer job before timeout ({timeout:?})")
            })
    });
    ...
    scan_thread
        ...
        .await
        .map_err(|e| eyre::eyre!("Could not join scan_thread: {e:?}"))?
}
