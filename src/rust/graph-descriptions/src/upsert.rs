...
fn escape_prop(node_property: &NodeProperty) -> String {
    match &node_property.property {
        Some(ProtoIncrementOnlyIntProp(i)) => i.to_string(),
        Some(ProtoDecrementOnlyIntProp(i)) => i.to_string(),
        Some(ProtoImmutableIntProp(i)) => i.to_string(),
        Some(ProtoIncrementOnlyUintProp(i)) => i.to_string(),
        Some(ProtoDecrementOnlyUintProp(i)) => i.to_string(),
        Some(ProtoImmutableUintProp(i)) => i.to_string(),
...
pub(crate) fn node_creation_quads(
    ...
    let escaped_node_key = escape_quote(node_key);
    let inner_query = format!(
        r#"
            {creation_var_name}(func: eq(node_key, {node_key}), first: 1) @cascade
            {{
                uid,
                node_key,
            }}
...
pub(crate) fn gen_upsert_quads(
    ...
    let mut inner_query = format!(
        r#"
            {node_query_name}(func: uid(creation_var_name), first: 1) @cascade
            {{
                uid,
                node_key,
            }}
    ...
    // If the node exists, set the predicate. Currently 'last write wins'.
    let mut mu_0_n_quads = format!(
        r#"uid(node_exists_{prop_name}) <{prop_name}> "{f}" ."#,
        prop_name = prop_name,
        f = prop_value,
    );

    mu_0.set_set_nquads(mu_0_n_quads);
    mu_0.set_cond("@if(eq(len(node_exists_{prop_name}), 1))");
    ...
    let mut mu_1_n_quads = format!(
        concat!(
            r#"_:node_{prop_name} <node_key> "{node_key}" ."#,
            "\n",
            r#"_:node_{prop_name} <{prop_name}> "{f}" ."#,
            "\n",
            r#"_:node_{prop_name} <dgraph.type> "{node_type}" ."#,
        ),
        node_key = node_key,
        node_type = node_type,
        prop_name = prop_name,
    ...
    mu_1.set_set_nquads(mu_1_n_quads);
    mu_1.set_cond(format!("@if(eq(len(node_exists_{prop_name}), 0))", prop_name=prop_name));
