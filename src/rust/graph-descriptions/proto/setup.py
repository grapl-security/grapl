...
def find_version():
    ...
    """
    with open(HERE + '/../Cargo.toml') as cargo_toml:
...
def compile_protobuf(proto_file, proto_dir):
...
def find_proto_files():
    ...
    for (base_path, _, files) in os.walk(HERE):
...
def find_generated_files():
    """find all the *_pb2.py files"""
    for (base_path, _, files) in os.walk(HERE):
        for file_name in files:
            if (
                    file_name.endswith('_pb2.py') or
...
def compile_all_protobufs():
    """walk the directory tree and compile every .proto file"""
    for proto_file in find_proto_files():
        compile_protobuf(proto_file, HERE)
    for (base_path, _, files) in os.walk(HERE):
...
class clean(_clean):
    def run(self):
        for generated_file in find_generated_files():
