...
def find_proto_files():
    """find all the .proto files in PROTO_DIR"""
...
def find_generated_files():
    """find all the *_pb2.py files in PROTO_DIR"""
    for (base_path, _, files) in os.walk(PROTO_DIR):
        for file_name in files:
            if (
...
def compile_all_protobufs():
    """walk the PROTO_DIR and compile every .proto file"""
    for proto_file in find_proto_files():
        compile_protobuf(proto_file, PROTO_DIR)
    for (base_path, _, files) in os.walk(PROTO_DIR):
