...
impl<S, const WRITE_CAP: bool, const LIST_CAP: bool> ReadS3<S, WRITE_CAP, LIST_CAP> for Bucket<S, true, WRITE_CAP, LIST_CAP>
    where S: S3Common
{
...
mod tests {
    ...
    async fn test_read_objects() -> Result<(), Box<dyn std::error::Error>> {
        ...
        bucket.put_object("key-0".to_owned(), vec![123])
            .send().await?;

        let object = bucket.get_object("key-0".to_owned())
            .send().await?;
