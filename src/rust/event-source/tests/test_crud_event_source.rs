...
async fn test_create_update_get() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let create_response = {
        ...
        client.create_event_source(request).timeout(common_timeout.clone()).await??
    };
    ...
    let get_response = {
        let request = es_api::GetEventSourceRequest{event_source_id: create_response.event_source_id};
        client.get_event_source(request).timeout(common_timeout.clone()).await??
    };
    ...
    let update_response = {
        ...
        client.update_event_source(request).timeout(common_timeout.clone()).await??
    };
