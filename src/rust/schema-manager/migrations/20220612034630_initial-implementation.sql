CREATE SCHEMA IF NOT EXISTS schema_manager;

CREATE OR REPLACE FUNCTION schema_manager.megabytes(bytes integer) RETURNS integer AS
$$
BEGIN
...
CREATE TABLE schema_manager.node_identity_algorithm
(
...
CREATE TABLE schema_manager.session_identity_arguments
(
    tenant_id                      uuid     NOT NULL,
    ...
    FOREIGN KEY (identity_algorithm, tenant_id, node_type, schema_version)
        REFERENCES schema_manager.node_identity_algorithm (identity_algorithm, tenant_id, node_type, schema_version) ON DELETE CASCADE
);


CREATE TABLE schema_manager.node_schemas
(
    tenant_id            uuid      NOT NULL,
    identity_algorithm   text      NOT NULL CHECK (identity_algorithm IN ('session', 'static')),
    node_type            text      NOT NULL CHECK (node_type <> '' AND length(node_type) < 32),
    ...
    raw_schema           bytea     NOT NULL CHECK (length(raw_schema) < schema_manager.megabytes(1)),
    ...
    FOREIGN KEY (tenant_id, node_type, schema_version, identity_algorithm)
        REFERENCES schema_manager.node_identity_algorithm (tenant_id, node_type, schema_version, identity_algorithm) ON DELETE CASCADE
);


CREATE TABLE IF NOT EXISTS schema_manager.static_identity_arguments
(
    tenant_id             uuid     NOT NULL,
...
CREATE TYPE schema_manager.property_type AS ENUM (
    'ImmutableString',
    'ImmutableI64',
    'MaxI64',
    'MinI64',
...
CREATE TABLE IF NOT EXISTS schema_manager.property_schemas
(
    tenant_id      uuid                         NOT NULL,
    node_type      text                         NOT NULL CHECK (node_type <> '' AND length(node_type) < 32),
    schema_version smallint                     NOT NULL,
    property_name  text                         NOT NULL CHECK (property_name <> '' AND length(property_name) < 32),
    property_type  schema_manager.property_type NOT NULL,
    ...
    FOREIGN KEY (tenant_id, node_type, schema_version)
        REFERENCES schema_manager.node_schemas (tenant_id, node_type, schema_version)
);

CREATE TYPE schema_manager.edge_cardinality AS ENUM ('ToMany', 'ToOne');

CREATE TABLE IF NOT EXISTS schema_manager.edge_schemas
(
    tenant_id                uuid                            NOT NULL,
    node_type                text                            NOT NULL CHECK (node_type <> '' AND length(node_type) < 32),
    schema_version           smallint                        NOT NULL,
    ...
    forward_edge_cardinality schema_manager.edge_cardinality NOT NULL,
    reverse_edge_cardinality schema_manager.edge_cardinality NOT NULL,
