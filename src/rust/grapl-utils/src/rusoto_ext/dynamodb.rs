...
use rusoto_dynamodb::{BatchGetItemInput, BatchGetItemError, BatchGetItemOutput, DynamoDb, AttributeValue, KeysAndAttributes};
...
pub trait GraplDynamoDbClientExt: DynamoDb {
    ...
    */
    async fn batch_get_item_reliably(&self, batch_get_items_input: BatchGetItemInput) -> Result<BatchGetItemOutput, RusotoError<BatchGetItemError>> {
        /*
        ...
        let mut pending_items: Vec<(String, HashMap<String, AttributeValue>)> = batch_get_items_input.request_items.into_iter()
            .flat_map(|(table, mut keys_and_attributes)| {
                let request_items: Vec<_> = keys_and_attributes.keys.drain(0 .. keys_and_attributes.keys.len()).collect();
        ...
        while !pending_items.is_empty() {
            ...
            let mut requested_items_for_table: HashMap<String, Vec<HashMap<String, AttributeValue>>> = HashMap::new();
            ...
            pending_items.drain(0 .. std::cmp::min(pending_items.len(), DYNAMODB_MAX_BATCH_GET_ITEM_SIZE))
                .for_each(|(table_name, row_keys)| {
                    let entry = requested_items_for_table.entry(table_name.clone())
                        .or_insert(Vec::new());
            ...
            let manufacted_request_items = requested_items_for_table.into_iter()
                .map(|(table_name, pending_items)| {
                    // grab the previously kept KeysAndAttributes to copy properties from
                    let keys_and_attributes_template = match key_and_attribute_shells.get(&table_name) {
            ...
            match response.responses {
                Some(response_map) => {
                    ...
                    response_map.into_iter()
                        .for_each(|(table_name, rows)| {
                            let table_rows = total_responses.entry(table_name.clone())
                                .or_insert(Vec::new());
            ...
            match response.unprocessed_keys {
                Some(unprocessed_keys) => {
                    let unprocessed_keys: Vec<_> = unprocessed_keys.into_iter()
                        .flat_map(|(table_name, keys_and_attributes)| {
                            keys_and_attributes.keys.into_iter()
        ...
        Ok(BatchGetItemOutput {
            consumed_capacity: batch_get_items_input.return_consumed_capacity.map(|_| total_consumed_capacity),
