use crate::graplinc::grapl::api::graph_query_service::v1beta1::messages::{QueryGraphFromNodeRequest, QueryGraphFromNodeResponse, QueryGraphWithNodeRequest, QueryGraphWithNodeResponse};
use crate::protobufs::graplinc::grapl::api::graph_query_service::v1beta1::graph_query_service_client::GraphQueryServiceClient;
use crate::protobufs::graplinc::grapl::api::graph_query_service::v1beta1::{
...
impl GraphQueryClient {
    ...
    pub async fn query_graph_with_uid(
        ...
        Ok(self.inner.query_graph_with_uid(request).await.map_err(Status::from)?.into_inner().try_into()?)
    }
    pub async fn query_graph_from_uid(
        ...
        Ok(self.inner.query_graph_from_uid(request).await.map_err(Status::from)?.into_inner().try_into()?)
    }
}
