...
use crate::{
    ...
    protobufs::graplinc::grapl::api::lens_subscription_service::v1beta1::{
        lens_subscription_service_client::LensSubscriptionServiceClient,
...
impl LensSubscriptionClient {
    ...
    pub async fn subscribe_to_lens(
        ...
        impl Stream<Item = Result<SubscribeToLensResponse, LensSubscriptionClientError>>,
        LensSubscriptionClientError,
    > {
        let request: SubscribeToLensRequestProto = request.into();
        let response = self
            .inner
            .subscribe_to_lens(tonic::Request::new(request))
            .await
            .map_err(Status::from)?;
        let response = response.into_inner();
        let response = StreamExt::map(response, |response| match response {
