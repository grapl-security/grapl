    ...
    D: GraplDynamoDbClientExt,
{
    ...
    pub async fn make_create_time_canonical(&self, session: &Session) -> Result<(), Error> {
        ...
        let upd_req = UpdateItemInput {
            ...
            update_expression: Some(
                "SET is_create_canon = :canon_is_create, version = :new_version".to_string(),
            ),
            table_name: self.table_name.clone(),
    ...
    pub async fn update_session_end_time(
        ...
        let upd_req = UpdateItemInput {
            ...
            update_expression: Some(
                "SET end_time = :end_time, is_end_canon = :is_end_canon, version = :new_version"
                    .to_string(),
            ),
            table_name: self.table_name.clone(),
    ...
    async fn create_sessions_from_unid_session_nodes(
        &self,
        unid_sessions: Vec<UnidSessionNode>,
        is_create_canon: bool,
    ) -> Result<Vec<AttributedNode>, Error> {
        let (newly_attributed_nodes, sessions): (Vec<_>, Vec<_>) = unid_sessions
            .into_iter()
    ...
    pub async fn create_sessions(&self, sessions: Vec<Session>) -> Result<(), Error> {
        let write_operations: Vec<_> = sessions
            .into_iter()
            .map(|session| PutRequest {
                item: serde_dynamodb::to_hashmap(&session).unwrap(),
            })
            .map(|put| WriteRequest {
                delete_request: None,
        ...
        self.dynamo
            .batch_write_item_reliably(batch_write_input)
            .await?;
    ...
    {
        let unid_queries: Vec<_> = unid_session_nodes
            .iter()
        ...
        let identification_results = query_results
            .into_iter()
    ...
    async fn find_first_sessions_after(
        ...
        let create_query_for_after_timestamp = |unid_session: &UnidSession| QueryInput {
        ...
        self.find_sessions_from_queries(creation_unids, create_query_for_after_timestamp)
            .await
    }
    ...
    async fn find_last_sessions_before(
        ...
        let create_query_for_before_timestamp = |unid_session: &UnidSession| QueryInput {
            consistent_read: Some(true),
            limit: Some(1),
        ...
        self.find_sessions_from_queries(last_seen_unids, create_query_for_before_timestamp)
            .await
    }

    fn split_session_pairs(
        &self,
        session_results: Vec<(UnidSessionNode, Option<Session>)>,
    ) -> (Vec<(UnidSessionNode, Session)>, Vec<UnidSessionNode>) {
        session_results
            .into_iter()
            .partition_map(|(session_node, session)| match session {
        ...
        for (UnidSessionNode(mut node_desc, unid_session), session) in first_after_session_pairs {
            let attributed_node = match skewed_cmp(unid_session.timestamp, session.create_time) {
                ...
                _ if !session.is_create_canon && session.create_time != unid_session.timestamp => {
                    ...
                    self.update_session_create_time(&session, unid_session.timestamp, true)
                        .await?;
    ...
    async fn process_last_before_session_nodes(
        ...
        for (UnidSessionNode(mut node_desc, unid_session), session) in last_before_session_pairs {
            let attributed_node = match !session.is_end_canon {
                _ if unid_session.timestamp < session.end_time
                    || skewed_cmp(unid_session.timestamp, session.end_time) =>
                {
                ...
                true => {
                    ...
                    self.update_session_end_time(&session, unid_session.timestamp, false)
                        .await?;
    ...
    async fn handle_creation_unid_sessions(
        ...
        let (first_after_attributed_nodes, no_session_nodes) = {
            let first_after_session_results =
                self.find_first_sessions_after(creation_unids).await?;

            let (first_after_session_pairs, no_session_nodes) =
                self.split_session_pairs(first_after_session_results);

            let first_after_attributed_nodes = self
                .process_first_after_session_nodes(first_after_session_pairs)
                .await?;

            (first_after_attributed_nodes, no_session_nodes)
        };

        let (last_before_attributed_nodes, no_session_nodes) = if !no_session_nodes.is_empty() {
            let last_before_session_results =
                self.find_last_sessions_before(no_session_nodes).await?;

            let (last_before_session_pairs, no_session_nodes) =
                self.split_session_pairs(last_before_session_results);

            let last_before_attributed_nodes = self
                .process_last_before_session_nodes(last_before_session_pairs)
                .await?;
        ...
        let newly_attributed_nodes = self
            .create_sessions_from_unid_session_nodes(no_session_nodes, true)
            .await?;

        let all_results: Vec<_> = newly_attributed_nodes
            .into_iter()
    ...
    async fn handle_last_seen_unid_sessions(
        ...
        let (first_after_attributed_nodes, no_session_nodes) = if !no_session_nodes.is_empty() {
            let first_after_session_results =
                self.find_first_sessions_after(no_session_nodes).await?;
        ...
        let newly_attributed_nodes = match no_session_nodes {
            session_nodes if !session_nodes.is_empty() && should_default => {
                self.create_sessions_from_unid_session_nodes(session_nodes, false)
                    .await?
            }
        ...
        let results: Vec<_> = first_after_attributed_nodes
            .into_iter()
            .chain(last_before_attributed_nodes)
    ...
    pub async fn identify_unid_session_nodes(
        ...
        let (creation_unids, last_seen_unids): (Vec<_>, Vec<_>) = unids
            .into_iter()
            .map(|mut entry| {
        ...
        let identified_last_seen_nodes = self
            .handle_last_seen_unid_sessions(last_seen_unids, should_default)
            .await?;

        let identified_nodes = identified_creation_nodes
            .into_iter()
