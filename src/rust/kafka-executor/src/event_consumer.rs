use rdkafka::consumer::{StreamConsumer, DefaultConsumerContext, ConsumerContext, Consumer};
use rdkafka::util::{DefaultRuntime, AsyncRuntime};
...
{
    fn from_env() -> Self {
        let topic = std::env::var(KAFKA_CONSUMER_TOPIC)
            .expect(KAFKA_CONSUMER_TOPIC);
        let brokers = std::env::var(KAFKA_CONSUMER_BROKERS)
            .expect(KAFKA_CONSUMER_BROKERS);
        let consumer_group_id = std::env::var(KAFKA_CONSUMER_GROUP_ID)
            .expect(KAFKA_CONSUMER_GROUP_ID);
        let consumer_client_id = std::env::var(KAFKA_CONSUMER_CLIENT_ID)
            .expect(KAFKA_CONSUMER_CLIENT_ID);
        ...
        let consumer = StreamConsumer::from_config(&client_config)
            .expect("StreamConsumer::from_config");
        consumer
            .subscribe(&[&topic])
