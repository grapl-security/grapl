...
use model_plugin_deployer::client::{Channel, Timeout};
use tonic::transport::NamedService;
use model_plugin_deployer::server::ModelPluginDeployerRpcServer;
use model_plugin_deployer::server::ModelPluginDeployer;
...
impl AsyncTestContext for ServiceContext {
    async fn setup() -> Self {
        ...
        tokio::task::spawn(async move {
            model_plugin_deployer::server::exec_service().await
...
async fn _healthcheck() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let request = HealthCheckRequest {
        service: ModelPluginDeployerRpcServer::<ModelPluginDeployer>::NAME.to_string(),
    };
