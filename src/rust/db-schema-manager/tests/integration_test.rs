...
async fn test_provision() -> Result<(), DynError> {
    ...
    let mut db_schema_manager_client = DbSchemaManagerClient::connect(db_schema_manager_endpoint).await?;
    tracing::info!("connected to graph query service");

    let tenant_id = uuid::Uuid::new_v4();
    _span.record("tenant_id", &format!("{tenant_id}"));

    db_schema_manager_client.query_graph_with_uid(DeployGraphSchemasRequest {
        tenant_id,
    }).await?;
