...
impl PreparedInsert {
    ...
    pub(crate) async fn prepare(
        ...
        let max_i64 = scylla_client
            .prepare(raw_property_prepare_statement(
                tenant_id,
                MAX_I_64_TABLE_NAME,
            ))
            .await?;
        let min_i64 = scylla_client
            .prepare(raw_property_prepare_statement(
                tenant_id,
                MIN_I_64_TABLE_NAME,
            ))
            .await?;
        let imm_i64 = scylla_client
            .prepare(raw_property_prepare_statement(
                tenant_id,
                IMM_I_64_TABLE_NAME,
            ))
            .await?;
        let max_u64 = scylla_client
            .prepare(raw_property_prepare_statement(
                tenant_id,
                MAX_U_64_TABLE_NAME,
            ))
            .await?;
        let min_u64 = scylla_client
            .prepare(raw_property_prepare_statement(
                tenant_id,
                MIN_U_64_TABLE_NAME,
            ))
            .await?;
        let imm_u64 = scylla_client
            .prepare(raw_property_prepare_statement(
                tenant_id,
                IMM_U_64_TABLE_NAME,
            ))
            .await?;
        let imm_string = scylla_client
            .prepare(raw_property_prepare_statement(
                tenant_id,
                IMM_STRING_TABLE_NAME,
            ))
