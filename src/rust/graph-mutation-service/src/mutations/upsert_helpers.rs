...
pub(crate) fn gen_mutations(
    mutations: &mut Vec<dgraph_tonic::Mutation>,
    node_exists: &str,
    set_query_name: &str,
    cmp_query_name: &str,
    prop_name: &str,
    ...
    mu_1.set_cond(format!(
        "@if(eq(len({node_exists}), 1) AND eq(len({set_query_name}), 0))",
        node_exists = node_exists,
        set_query_name = set_query_name
    ));
    ...
    let mu_2_n_quads = format!(
        concat!(r#"_:{node_exists} <{prop_name}> {prop_value} ."#,),
        node_exists = node_exists,
        prop_name = prop_name,
        prop_value = prop_value,
    );
    ...
    mu_2.set_cond(format!(
        "@if(eq(len({node_exists}), 0))",
        node_exists = node_exists
    ));
...
pub(crate) fn gen_immutable_query(
    query_buffer: &mut String,
    creation_var_name: &str,
    node_id: u128,
    query_suffix: &str,
    ...
    let query_name = format!(
        "exists_but_unset_{query_suffix}",
...
pub(crate) fn gen_immutable_mutations(
    mutations: &mut Vec<dgraph_tonic::Mutation>,
    node_exists: &str,
    query_name: &str,
    prop_name: &str,
    ...
    mu_0.set_set_nquads(mu_0_n_quads);
    mu_0.set_cond(format!(
        "@if(eq(len({node_exists}), 1) AND eq(len({query_name}), 0))",
        node_exists = node_exists,
        query_name = query_name
    ));
