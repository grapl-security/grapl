...
impl NodeUpsertGenerator {
    pub fn generate_create_node(&mut self, node_type: String) -> dgraph_tonic::Mutation {
        let mut mu_1 = dgraph_tonic::Mutation::new();
        let mu_1_n_quads = format!(
            concat!(
            r#"_:create_node <dgraph.type> "{node_type}" ."#,
            ),
            node_type = node_type,
        );
    ...
    pub fn generate_upserts(
        ...
        let uid = Escaped::from(value.uid);
        let predicate_names = value.properties.keys().collect::<Vec<_>>();
        let creation_query_name = self.gen_query(node_id, &uid, &predicate_names);
        self.node_creation_quads(&creation_query_name, &value.node_type);
