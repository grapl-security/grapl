...
def list_dlqs_for_deployment(
    ...
    queues = [q for q in queues if q.endswith(DLQ_SUFFIX)]
    return queues
...
def redrive_from_dlq(
    ...
    assert dlq_queue_url.startswith(
        "http"
    ), "Please pass a Queue URL (try a `graplctl queues ls`)."
    assert dlq_queue_url.endswith(
        DLQ_SUFFIX
    ), "You're trying to redrive from a non-dead-letteAr-queue."
...
def _retry_queue_for_dlq(queue_url: str) -> str:
