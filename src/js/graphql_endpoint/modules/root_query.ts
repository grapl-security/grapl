...
const handleLensScope = async (
  ...
  const asEnrichedNode = (node: RawNode) => asEnrichedNodeWithSchemas(node, schemaMap);
  ...
  const uids_in_scope = new Set<number>(scope.map((node: EnrichedNode) => node.uid));
  ...
  for (const node of scope) {
    ...
    for (const predicate in node) {
      ...
      if (Array.isArray(node[predicate]) && node[predicate] && node[predicate][0]["uid"]) {
        node[predicate] = batchEnrichNodes(node[predicate]).filter(
          (neighbor_of_node: EnrichedNode) => uids_in_scope.has(neighbor_of_node["uid"]),
        );
      ...
      else if (typeof node[predicate] === "object" && node[predicate]["uid"]) {
    ...
    fields: {
      ...
      lens_scope_query: {
        ...
        resolve: async (parent: never, args: never) => {
          ...
          const query_string = new QueryGenerator(GraplEntityType).generate();
          return {
            query_string,
          };
        },
