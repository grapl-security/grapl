...
describe("QueryGenerator", () => {
  ...
  describe("genOnFragment", () => {
    ...
    it("generates Risk correctly", () => {
      expect(queryGen.genOnFragment({ type: Risk, numSpaces: 0 })).toEqual(expectedRisk);
    });
    ...
    it("prepends spaces", () => {
      const expectedRiskWithSpaces = expectedRisk
        ...
        .map((s) => `    ${s}`)
        .join("\n");
      expect(queryGen.genOnFragment({ type: Risk, numSpaces: 4 }).trim()).toEqual(
        expectedRiskWithSpaces.trim(),
      );
    });
  });
